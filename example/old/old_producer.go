...
func constructFakeBasket() (pb_Basket types.Pb_Basket, eventTimestamp time.Time, storeName string, err error) {
	...
	for count := 0; count < nBasketItems; count++ {
		...
		var BasketItem types.BasketItem
		BasketItem.Id = varSeed.Products[productId].Id
		BasketItem.Name = varSeed.Products[productId].Name
		BasketItem.Brand = varSeed.Products[productId].Brand
		BasketItem.Category = varSeed.Products[productId].Category
		BasketItem.Price = varSeed.Products[productId].Price
		BasketItem.Quantity = int32(quantity)
	...
	pb_Basket.InvoiceNumber = txnId
	pb_Basket.SaleDateTime = eventTime
	pb_Basket.SaleTimestamp = fmt.Sprint(eventTimestamp.UnixMilli())
	pb_Basket.Store = &store
	pb_Basket.Clerk = &clerk
	pb_Basket.TerminalPoint = strconv.Itoa(terminalPoint)
	pb_Basket.BasketItems = BasketItems
	pb_Basket.Nett = nett_amount
	pb_Basket.Vat = vat_amount
	pb_Basket.Total = total_amount

	fmt.Println(reflect.TypeOf(pb_Basket))
